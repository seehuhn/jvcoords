---
title: "Notes for the JvMult Package"
output:
  html_notebook:
    number_sections: yes
    toc: yes
...
It seems that the `scale()` function is not an efficient way to standardise
data:
...
We list the code to transform (additional) data to and from the space of
...
Here we use an approach to compute $Y = C^{-1/2}
(X - \bar X)$, where $X - \bar X$ denotes the matrix X with the column means
subtracted from each column, and $C = \mathrm{Cov}(X)$:  Using SVD
we can write $X - \bar X$ as $X - \bar X = U D V^\top$ and thus we get
$$
C
= \mathrm{Cov}(X - \bar X)

coords <- function(loadings, pre.sub = 0, pre.div = 1, post.mul = 1) {
    # nrow(loadings) == p
    # ncol(loadings) == q
    ...
    res <- list(loadings = loadings, pre.sub = pre.sub, pre.div = pre.div, post.mul = post.mul)
    class(res) <- "coords"
    res
}
...
fromCoords <- function(trans, y) {
    if (is.null(dim(y))) {
        stopifnot(length(y) == ncol(trans$loadings))
        y <- y/trans$post.mul
...
print.coords <- function(x, ...) {
    ...
    cat("mapping p =", nrow(x$loadings), "coordinates to q =", ncol(x$loadings),
        "coordinates\n")

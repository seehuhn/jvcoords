jvICA <- function(x, n.comp, maxit = 5000) {
    ...
    # IC is an orthogonal matrix.  The following loop will modify IC to produce the
    # independent components in the columns of IC one by one, while keeping the
    # matrix orthogonal.
    IC <- diag(q)
    # If the remaining search space is one-dimensional (i.e. if k=q), no search is
    # required and the corresponding column of IC already contains a normed vector
    # in this space.  Thus we need to find at most q-1 directions.
    for (k in 1:min(n.comp, q - 1)) {
        # Find the direction in the space spanned by IC[,k], ..., IC[,q] which
        # minimises entropy.

        r <- q - k + 1  # the dimension of the search space

        # step 1: Try random directions to find a good starting point for the
        # optimisation.
        ...
        # step 2: use local optimisation to find the best solution in the vincinity of
        # best.dir
